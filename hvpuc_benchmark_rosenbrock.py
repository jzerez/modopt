...
def jax_hess(x, beta=10):
    n = x.size
    h = np.zeros((n, n))
    for i in range(n):
        if i == 0:
            d = 12*beta*x[i]**2 - 4*beta*x[i+1] + 2
            od = -4 * beta * x[i]
            h[i, i+1] = od
            h[i+1, i] = od
        elif i == n-1:
            ...
            d = 2*beta + 12 * beta * x[i]**2 - 4 * beta * x[i+1] + 2
            od = -4 * beta * x[i]
            h[i, i+1] = od
            h[i+1, i] = od
...
prob_name = f'rosenbrock_{n}_{beta}_1'
...
algs=  ['BFGS', 'iBFGS-1-s', 'iBFGS-10-s', 'iBFGS-20-s', 'iBFGS-40-s'
...
run_name = f'rosenbrock_{n}_{beta}_1'
save_eval_df = True
save_errs = True
err_hist = {}
...
for alg_num, alg in enumerate(algs):
    ...
    try:
        for i in range(1):
            ...
            if solver == 'OpenSQP':
                ...
                if plot_on:
                    ...
                    ax = axs[0][0]
                    true_hess = [jax_hess(x, beta) for x in results['x_history'].T]
                    ...
                    nevs = np.array(results['nfevs']) + np.array(results['ngevs']) + np.array(results['n_hvps'])
                    ax.plot(nevs, [np.linalg.norm(bk - hk) for bk, hk in zip(results['approx_hess'], true_hess)], linestyle=ls, label=alg+r' $B_k$ norm')
                    # ax.plot([np.linalg.norm(bk - hk) for bk, hk in zip(results['qn_hess'], results['true_hess'])], label=r'QN $B_k$ norm')
                    ax.set_yscale('log')
                    ax.set_ylabel(r'$||B_k - H_k||_F$')
                    ax.set_xlabel('Number of Evaluations')
                    # ax.set_title(r'$B_k$ Error')
                    # ax.legend(fontsize=18, bbox_to_anchor=(1.05, 1), loc='upper left', borderaxespad=0.)
                    ax.legend()

                    ax = axs[0][1]
                    ax.plot([np.linalg.norm(bk - hk) for bk, hk in zip(results['approx_hess'], true_hess)], linestyle=ls, label=alg+r' $B_k$ norm')
                    # ax.plot([np.linalg.norm(bk - hk) for bk, hk in zip(results['qn_hess'], results['true_hess'])], label=r'QN $B_k$ norm')
                    ax.set_yscale('log')
                    ax.set_ylabel(r'$||B_k - H_k||_F$')
                    ax.set_xlabel('Number of Iterations')
                    # ax.set_title(r'$B_k$ Error vs. Iterations')
                    ax.legend()

                    ax = axs[1][1]
                    ax.plot(results['objectives'], linestyle=ls, label=alg+r' $B_k$ norm')
                    # ax.plot([np.linalg.norm(bk - hk) for bk, hk in zip(results['qn_hess'], results['true_hess'])], label=r'QN $B_k$ norm')
                    ax.set_yscale('log')
                    ax.set_ylabel(r'optimality')
                    ax.set_xlabel('Number of Iterations')
                    # ax.set_title(r'Optimality vs. Iterations')
                    ax.legend()

                    ax = axs[1][0]
                    ax.plot(nevs, results['objectives'], linestyle=ls, label=alg+r' $B_k$ norm')
                    # ax.plot([np.linalg.norm(bk - hk) for bk, hk in zip(results['qn_hess'], results['true_hess'])], label=r'QN $B_k$ norm')
                    ax.set_yscale('log')
                    ax.set_ylabel(r'optimality')
                    ax.set_xlabel('Number of Evaluations')
        ...
        opt_time = (time.time() - start_time) / 1
        success  = results['success']
        print(success)
